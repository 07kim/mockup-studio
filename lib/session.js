...
export async function uploadSession({ id, files }) {
  ...
  if (!inputs.length) {
    throw Object.assign(new Error('このページにファイルの入力欄が見つかりませんでした。サイト側の「ファイルを選択」などが表示されている画面で試してください。'), { code: 'NO_FILE_INPUT' });
  }
  ...
  try {
    await target.setInputFiles(mapped);
  } catch (e) {
    // 単一選択の input に複数渡すと失敗するので、先頭1件で再試行。
    if (mapped.length > 1) await target.setInputFiles([mapped[0]]);
    else throw e;
  }
  await s.page.waitForTimeout(200).catch(() => {}); // プレビュー・自動アップロードの反映待ち
  return snap(s);
}
