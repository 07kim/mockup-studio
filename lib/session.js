...
/** ページ個別のイベント（ファイル選択の掴み取り）を仕掛ける。 */
function attachPageHandlers(s, page) {
  page.on('filechooser', (fc) => { s.pendingChooser = fc; });
}

/**
 * 生きている session を取得。
 * 現在のページが閉じていても、同じコンテキストに別ページ（ポップアップ/新タブ）が
 * 残っていればそれに切り替える（GAS等の画面遷移でセッションが切れないように）。
 */
function liveSession(id) {
  const s = sessions.get(id);
  if (!s) throw sessionGone();
  if (s.page.isClosed && s.page.isClosed()) {
    const alive = s.context.pages().filter((p) => !(p.isClosed && p.isClosed()));
    if (alive.length) { s.page = alive[alive.length - 1]; return s; }
    sessions.delete(id); throw sessionGone();
  }
  return s;
}
...
export async function openSession({ url, width = 1440, height = 900, dpr = 2, mobile = false }) {
  ...
  attachPageHandlers(s, page);
  // ポップアップ/新タブが開いたら、その新しいページに追従する（画面遷移で切れないように）。
  context.on('page', (p) => { s.page = p; s.last = Date.now(); attachPageHandlers(s, p); });
  sessions.set(id, s);
...
async function applyEvent(page, e) {
  if (e.t === 'click') await page.mouse.click(e.x, e.y);
    ...
    // カーソル位置へ移動してからホイール。ページ全体だけでなく、その位置にある
    // 内側のスクロール領域（モーダル・チャット欄など）も正しくスクロールできる。
    if (typeof e.x === 'number' && typeof e.y === 'number') await page.mouse.move(e.x, e.y).catch(() => {});
    await page.mouse.wheel(e.dx || 0, e.dy || 0);
  }
