...
import { getFrame, calcNativeScale } from './devices.js';
...
function drawScreen(ctx, img, screen, ri, settings, rs) {
  ...
  if (img && img.width) {
    // フレームは固定のまま、画面内の画像だけをズーム／移動する（§画像の位置・大きさ）。
    // 重要: 元画像を切り抜かずに「画像全体」を拡大・配置し、はみ出しは clip で切る。
    // こうすると移動（imgX/imgY）で切れた部分（続き）が実際に現れる。
    const z = settings.imgZoom ?? 1;
    const ox = ((settings.imgX ?? 0) / 100) * screen.w;
    const oy = ((settings.imgY ?? 0) / 100) * screen.h;
    if (settings.fit === 'stretch') {
      // 引き伸ばし: 画面いっぱいに変形（ズーム・移動も適用）。
      const cx = screen.x + screen.w / 2, cy = screen.y + screen.h / 2;
      ctx.translate(cx + ox, cy + oy);
      ctx.scale(z, z);
      ctx.translate(-cx, -cy);
      ctx.drawImage(img, Math.round(screen.x), Math.round(screen.y), Math.round(screen.w), Math.round(screen.h));
    } else {
      // ベース倍率: contain=全体が収まる / cover(既定)=画面を覆う。
      const rImg = img.width / img.height;
      const rBox = screen.w / screen.h;
      const isAspectMatch = Math.abs(rImg - rBox) / (rBox || 1) < 0.01;
      const base = isAspectMatch
        ? screen.w / img.width
        : (settings.fit === 'contain')
          ? (rImg > rBox ? screen.w / img.width : screen.h / img.height)
          : (rImg > rBox ? screen.h / img.height : screen.w / img.width);
      const dw = img.width * base * z;
      const dh = img.height * base * z;
      const dx = screen.x + (screen.w - dw) / 2 + ox;
      const dy = screen.y + (screen.h - dh) / 2 + oy;
      ctx.drawImage(img, Math.round(dx), Math.round(dy), Math.round(dw), Math.round(dh)); // 全体を描画→clipで画面矩形に収める
    }
  }
  ctx.restore();
}
...
function fitFrameToImage(spec, imageAspect) {
  if (spec.draw === 'browser') {
    const sw = spec.base.w;
    const barH = spec.barH ?? 38;
    const sh = sw / imageAspect;
    return { ...spec, base: { w: sw, h: sh + barH } };
  }
  const bez = spec.bez ?? 12;
...
function drawFace(frame, item, settings, rs) {
  ...
  // ブラウザ枠は、画像の縦横比に合わせてフレーム高さを自動調整（Webページを途中で切り取らない）
  if (spec.draw === 'browser' && item.img && item.img.width && item.img.height) {
    const ia = item.img.width / item.img.height;
    if (fit !== 'stretch') {
      spec = fitFrameToImage(spec, ia);
    }
  } else if (fit === 'frame' && item.img && item.img.width) {
    const ia = item.img.width / item.img.height;
    if ((frame.spec.draw || 'slab') === 'slab' || frame.spec.draw === 'browser') {
      spec = fitFrameToImage(frame.spec, ia);
    } else {
      fit = 'contain';
    }
  }
...
export function renderItem(item, settings, rs, N, bgImg) {
  ...
  let effRs = rs;
  if (effRs === 'auto' || effRs == null || typeof effRs !== 'number' || Number.isNaN(effRs)) {
    if (settings && typeof settings.scale === 'number') effRs = settings.scale;
    else effRs = calcNativeScale(item, frame);
  }
  const face = drawFace(frame, item, settings, effRs);
  const tilt = build3D(face.canvas, settings, N, effRs, face.ro || 0);

  const dw = tilt.canvas.width;
  const dh = tilt.canvas.height;
  const shadow = settings.shadow ? Math.max(20, 40 * effRs) * (settings.shadowStr ?? 0.55) : 0;
  // 余白 0 なら本当にフレームぴったり。影ONのときだけ影が切れないよう最小余白を足す。
  const margin = (settings.pad || 0) * effRs + (settings.shadow ? shadow * 0.7 : 0);
