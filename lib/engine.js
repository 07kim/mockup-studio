...
export function build3D(face, settings, N, rs, ro = 0) {
  ...
  // 遠近によるフォアショートニング（傾けると手前が大きく・奥が小さくなり、全体もやや縮む）を
  // そのまま活かす。以前は見かけの大きさを一定に正規化していたが、これが「上下で大きさが変わらない
  // ＝平面的なゆがみに見える」原因だったため廃止し、正しい3Dの見え方にする。

  // 角丸外周を前面/背面で投影（厚みの押し出し用）
  const outline = thick > 0 ? roundedRectOutline(W, H, ro, 10) : [];
  const projF = outline.map(([x, y]) => project(x - W / 2, y - H / 2, 0, rotX, rotY, f));
  const projB = outline.map(([x, y]) => project(x - W / 2, y - H / 2, -thick, rotX, rotY, f));
