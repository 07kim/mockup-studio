...
import { getFrame, captureViewportFor } from './devices.js';
...
export async function renderHtmlItem(item) {
  const vp = captureViewportFor(item.device, item.orientation);
  ...
  if (src.type === 'url') {
    ...
    fd.append('deviceScaleFactor', String(vp.dpr));
    if (vp.mobile) fd.append('mobile', 'true');
